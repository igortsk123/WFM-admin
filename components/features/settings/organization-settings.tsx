...
export function OrganizationSettings() {
  ...
  const [localPaymentMode, setLocalPaymentMode] = useState<PaymentMode>(
    org.payment_mode
  );
  ...
  async function handleConfirmSave() {
    ...
    try {
      ...
      if (result.success) {
        ...
        // TODO: propagate payment_mode change to AuthContext when real API is connected
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Freelance & Tariffs section — visible only when module is enabled */}
      {loading && !error && <FreelanceSectionSkeleton />}

      {!loading && !error && config && org.freelance_module_enabled && (
