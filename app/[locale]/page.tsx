...
const REVIEW_STATES: TaskReviewState[] = ["PENDING", "APPROVED", "REJECTED", "NEEDS_REVISION"]
...
const ROLES: FunctionalRole[] = ["NETWORK_OPS", "REGIONAL", "SUPERVISOR", "STORE_DIRECTOR", "HR_MANAGER", "OPERATOR", "WORKER", "AGENT", "PLATFORM_ADMIN"]
const WORK_TYPES = [
  ...
  return (
    <main className="mx-auto flex max-w-3xl flex-col gap-8 p-8 font-sans">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Раздел 2 — Status badges &amp; display primitives</p>
      </div>
      ...
      <Section title="ReviewStateBadge — task.review">
        {REVIEW_STATES.map((r) => (
          <ReviewStateBadge key={r} reviewState={r} />
        ))}
        <ReviewStateBadge reviewState="NONE" />
      </Section>
      ...
      <Section title="WorkTypeBadge — deterministic id % 6 → 6 colour schemes (light)">
        {WORK_TYPES.map((wt) => (
          <WorkTypeBadge key={wt.id} workType={wt} />
        ))}
      </Section>
      ...
      <nav className="rounded-xl border border-border bg-card p-6">
        <p className="mb-3 text-sm font-medium text-muted-foreground">{tc("language")}: RU / EN</p>
        <ul className="flex flex-col gap-2 text-sm">
          {(["dashboard", "tasks", "goals", "schedule", "employees"] as const).map((key) => (
            <li key={key}>
              <Link href={`/${key}`} className="text-primary underline-offset-4 hover:underline">
                {t(key)}
              </Link>
            </li>
          ))}
