...
/**
 * Реализация аналитики через Yandex AppMetrica для HMS флейвора.
 */
class AppMetricaAnalyticsService : AnalyticsService {
    ...
    override fun track(event: AnalyticsEvent) {
        ...
        if (params.isEmpty()) {
            ...
            AppMetrica.reportEvent(event.name(), JSONObject(params).toString())
        }
    }
    ...
    override fun setUser(userId: Int, role: String) {
        ...
        val profile = UserProfile.newBuilder()
            .apply(Attribute.customString("user_role").withValue(role))
            .build()
