...
/**
 * Сервис для получения FCM push-уведомлений.
 *
 * Ответственности:
 * - onNewToken: регистрировать новый FCM-токен в svc_notifications
 * - onMessageReceived: показывать системное уведомление с deep link на задачу
 */
class WfmFirebaseMessagingService : FirebaseMessagingService() {

    companion object {
        private const val TAG = "WfmFCMService"
    ...
    /**
     * FCM-токен обновлён — сохраняем локально и пробуем зарегистрировать.
    ...
    override fun onNewToken(token: String) {
        Log.i(TAG, "Новый FCM токен получен")
    ...
    /**
     * Получено push-уведомление (data-only или notification+data).
    ...
    override fun onMessageReceived(message: RemoteMessage) {
        ...
        val intent = Intent(this, MainActivity::class.java).apply {
            ...
            putExtra(PushConstants.EXTRA_PUSH_CHANNEL, "fcm")
        }
