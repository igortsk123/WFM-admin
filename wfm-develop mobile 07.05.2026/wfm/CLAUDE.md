...
- **Reverse Proxy**: nginx (конфигурация в `backend/nginx/`; на сервере `/etc/nginx/services/` — symlink на `/srv/wfm/backend/nginx/services/`, изменения подхватываются через `git pull` + `nginx -t && systemctl reload nginx`, см. `.memory_bank/backend/guides/nginx.md`)
...
└── services/
    ...
    └── monitoring.conf        # Прокси для svc_monitoring (→ localhost:8004/)
```
...
- **Monitoring API**: `https://dev.wfm.beyondviolet.com/monitoring/`
- **Monitoring WS**: `wss://dev.wfm.beyondviolet.com/monitoring/api/events/ws`
...
- **Monitoring API**: `https://wfm.beyondviolet.com/monitoring/`
- **Monitoring WS**: `wss://wfm.beyondviolet.com/monitoring/api/events/ws`
...
- **Monitoring API**: `http://localhost:8004/`
- **Monitoring WS**: `ws://localhost:8004/api/events/ws`
...
- **Monitoring**: `/monitoring/docs` (ReDoc: `/monitoring/redoc`)
...
Сервис мониторинга хоста и Docker. Источник snapshot'а для физического ESP32-устройства, приёмник 5xx ошибок от других сервисов. Параллельно шлёт `server_metrics` и `docker_container_event` в Semetrics (исторический ряд).

**Public API (DEVICE_TOKEN — query или Bearer):**
- `GET /health` — healthcheck (без авторизации)
- `GET /api/state` — JSON snapshot {cpu, mem, disk, containers, errors_1h}

**WebSocket (DEVICE_TOKEN):**
- `WS /api/events/ws?token={...}` — push-канал: `state` каждые 30с + `alert` мгновенно

**Internal API (без JWT, только Docker-сеть):**
- `POST /internal/errors` — приём 5xx ошибок от svc_tasks/svc_users/svc_notifications

Подробнее см. `.memory_bank/backend/apis/api_monitoring.md` и `.memory_bank/backend/services/svc_monitoring.md`.
...
- `svc_tasks/svc_users/svc_notifications → svc_monitoring` — fire-and-forget отчёты о 5xx (`shared.monitoring_client`)
...
- Конфигурация: `USERS_SERVICE_URL`, `USERS_SERVICE_TIMEOUT`, `MONITORING_SERVICE_URL`, `MONITORING_SERVICE_TIMEOUT` env vars
...
- Обращайтесь к `.memory_bank/backend/` для спецификаций сервисов и API-контрактов
  ...
  - `api_monitoring.md` — API svc_monitoring (state, WS events, /internal/errors)
- Обращайтесь к `.memory_bank/device/` для физического ESP32-монитора инфраструктуры
  - `README.md` — концепция устройства (pocket/desk-режимы, общие принципы)
  - `ws_protocol.md` — формат WS-сообщений `state`/`alert` (источник правды контракта)
  - `hardware_specs.md` / `JC4827W543N_specs.md` — характеристики тест-платформы и активного устройства
  - `display_design_brief.md` — раскладка dashboard-экрана; прошивка живёт в `backend/svc_monitoring/firmware/`
...
- Обращайтесь к `.memory_bank/mobile/` для требований к iOS/Android платформам
  ...
  - `brands.md` — два бренда приложений: «ХВ Сотрудник» и «Умный Сотрудник» (bundle IDs, отличия, реализация по платформам)
...
- Обращайтесь к `.memory_bank/strategy/` для архитектурных обоснований и политик
  - `theoretical_foundation.md` — модель EIM и Δt как опора для будущих аналитических слоёв; справочник «почему текущие данные собираются именно так»
  - `ai_oversight_policy.md` — политика человеческого надзора над AI; чеклист до мерджа AI-фич
  - `decision_modes.md` — режимы работы FAST / STANDARD / FULL; **проверяй перед стартом задачи**, чтобы не запускать `Plan` на правку текста и не делать без него миграции
  - `decision_record.md` — формат ADR; **обязателен в FULL-режиме** (миграции БД, изменения межсервисного контракта, AI-фичи, новые роли/привилегии)
- Обращайтесь к `.memory_bank/analytics/dt_metric.md` за концепцией метрики Δt (длительность управленческого цикла) — не реализована, зафиксированы определение и источники
