...
## Заложенные расширения (не реализовано)

Раздел фиксирует архитектурные намерения, к которым продукт может вернуться при появлении конкретной потребности. **Эти поля не существуют в БД и не предполагаются в ближайших спринтах.**

### Photo metadata

При появлении AI-скоринга фотоотчётов потребуется контекст конкретного снимка: когда сделан, с какого устройства, что детектировал AI. Текущее поле `report_image_url` содержит только URL — это достаточно для ручной проверки, но недостаточно для обучения модели и для расследования спорных случаев.

**Возможные nullable-поля** (вводить вместе с первым AI-кейсом, не раньше):
- `photo_captured_at: datetime | null` — момент съёмки (отличается от `updated_at`, если фото переотправлялось)
- `photo_device_id: string | null` — идентификатор устройства съёмки
- `ai_score: float | null` — численная оценка AI (0..1)
- `ai_confidence: float | null` — уверенность модели
- `ai_labels: jsonb | null` — детектированные объекты / нарушения

**Обоснование:** исторические фото нельзя обогатить задним числом. Когда AI-слой появится, ему нужен будет хотя бы какой-то prefix истории с метаданными.

**Что не делаем заранее:**
- Не добавляем поля «на будущее» в текущую миграцию.
- Не вводим параллельную таблицу `photo_metadata` без бизнес-причины.
- Не закладываем `business_outcome` (нет источника заполнения).

См. `.memory_bank/strategy/ai_oversight_policy.md` — политика расширения AI-фич.
