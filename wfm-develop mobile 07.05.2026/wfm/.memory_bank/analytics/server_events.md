...
Параллельно тот же факт ошибки уходит в `svc_monitoring` через `POST /internal/errors` — для счётчика `errors_1h` и алерт-спайков на физическом мониторе. Контракт Semetrics-события не меняется. См. `.memory_bank/backend/apis/api_monitoring.md`.
